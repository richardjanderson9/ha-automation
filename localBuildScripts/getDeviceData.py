...
FILTER_SCRIPT = os.path.join(SCRIPT_DIR, "filterData.py")
...
async def get_ha_registry():
    ...
    if len(url) < 10 or not token:
        print(f"❌ Error: HA_URL or HA_TOKEN is invalid in scriptData.json")
        sys.exit(1)
    ...
    try:
        ...
        async with websockets.connect(url) as websocket:
            ...
            print("\n" + "━" * 40)
            choice = input("❓ Would you like to filter this data now? (y/n): ").strip().lower()
            
            if choice == 'y':
                term = input("🔍 Enter search term (e.g. Office, light, S25): ").strip()
                if term:
                    # Executes your new filterData.py script
                    subprocess.run([sys.executable, FILTER_SCRIPT, term])
                else:
                    print("⚠️ No search term entered. Skipping filter.")
            else:
                print("👋 Export complete. No filtering requested.")
